...
def _pick_currency_and_quotation(
    ...
    primary_quotation_type: Optional[QuotationType] = None
    if stocks:
        ...
        if not source.quotationTypeInvalid:
    ...
    if not primary_quotation_type:
        if stocks:
            quote_type_stock = next((s for s in stocks if not s.quotationTypeInvalid), None)
            if quote_type_stock:
...
def _synthesize_boundary_balances(
    ...
    *,
...
) -> Tuple[Decimal, Decimal, Optional[Decimal], date]:
    ...
    return opening_balance, closing_balance, closing_value, end_plus_one
...
def augment_list_of_securities(
    ...
    *,
    ...
    rights_issue_cleanup_eligible: Optional[Callable[[SecurityPayment], bool]] = None,
    ...
    for sec_pos_obj, data in positions.items():
        ...
        opening_balance, closing_balance, closing_value, end_plus_one = _synthesize_boundary_balances(
            sorted_stocks,
        ...
        security = Security(
            ...
            nominalValue=Decimal("1000") if primary_quotation_type == "PERCENT" and hints.security_category == "BOND" and hints.country in ["US", "CA"] else None,
        )
        depot_securities_map[sec_pos_obj.depot].append(security)

        if hints.is_rights_issue and opening_balance == 0 and closing_balance == 0 and security.country=="CH" and security.payment and (security.stock is None or sum(1 for s in security.stock if s.corpAction in (None, False)) <= 1):
            rights_issue_cleanup_map[sec_pos_obj.depot].append(security)
    ...
    if len(rights_issue_cleanup_map) > 0 and rights_issue_cleanup_eligible is not None:
        for depot_id, securities_in_depot in rights_issue_cleanup_map.items():
            for sec in securities_in_depot:
                ...
                for payment in sec.payment:
                    if rights_issue_cleanup_eligible(payment):
