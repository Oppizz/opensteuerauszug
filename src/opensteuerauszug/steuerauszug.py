    ...
    try:
        ...
        if Phase.CALCULATE in run_phases:
            ...
            if tax_calculation_level == TaxCalculationLevel.MINIMAL:
                ...
                tax_value_calculator = BrokerFillInTaxValueCalculator(mode=CalculationMode.OVERWRITE, exchange_rate_provider=exchange_rate_provider, keep_existing_payments=calculate_settings.keep_existing_payments, render_language=render_language, use_broker_exch_rate=use_broker_exch_rate)

            if tax_value_calculator and calculator_name:
                if calculate_settings.remove_zero_positions:
                ...
                if calculate_settings.summarize_options:
                    tax_value_calculator.summarize_options = True
                if calculate_settings.remove_offsetting_payments:
        ...
        if Phase.VERIFY in run_phases:
            ...
            if tax_calculation_level == TaxCalculationLevel.MINIMAL:
                ...
                tax_value_verifier = BrokerFillInTaxValueCalculator(mode=CalculationMode.VERIFY, exchange_rate_provider=exchange_rate_provider_verify, keep_existing_payments=calculate_settings.keep_existing_payments, render_language=render_language, use_broker_exch_rate=use_broker_exch_rate)
